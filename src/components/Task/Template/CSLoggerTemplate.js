  ...
  template: `
        ...
        <b-row v-if="userProgress < 100">
          ...
          <b-col md="7" class="order-1 order-md-2" 
            ...
            <div v-for="resource in taskInfo" :key="resource.id" class="text-center">
                <image-task-presenter
                  v-if="getMime(resource.link_raw || resource.url)=='img'"
                  :link="resource.url || resource.link_raw"
                  :pybossa="pybossa"
                  :loading="!pybossa.taskLoaded"
                  style="position: sticky;top: 10%;"
                />
                <media
                  v-else-if="getMime(resource.link_raw || resource.url)=='video'"
                  :link="resource.link_raw || resource.video_url"
                  type="video" :loading="!pybossa.taskLoaded"
                  style="position: sticky;top: 10%;">
                </media>
                <media v-else-if="getMime(resource.link_raw || resource.url)=='audio'"
                  :link="resource.link_raw || resource.audio_url"
