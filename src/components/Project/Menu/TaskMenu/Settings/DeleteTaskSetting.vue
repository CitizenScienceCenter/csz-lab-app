  ...
  <div>
    ...
    <b-container>
        ...
        <b-list-group>
          ...
          <b-list-group-item><b>{{ $t('description') }}</b>: {{ project.description }}</b-list-group-item>
          <b-list-group-item><b>{{ $t('creation-date') }}</b>: {{ projectCreationDate }}</b-list-group-item>
          <b-list-group-item><b>{{ $t('last-activity') }}</b>: {{ infos.last_activity }}</b-list-group-item>
          <b-list-group-item><b>{{ $t('overall-progress') }}</b>: {{ infos.overall_progress }}% {{ $t('completed') }}</b-list-group-item>
          <b-list-group-item><b>{{ $t('tasks-c') }}</b> ({{ infos.n_tasks }})</b-list-group-item>
        </b-list-group>

        <p class="mt-4 quote">{{ $t('task-settings-delete-confirm-note') }}</p>
        <b-btn ref="btn-delete" class="mt-2 mt-md-0" @click="deleteTasks" variant="danger">{{$t('yes-c')}}</b-btn>
        <b-btn ref="btn-go-back" class="mt-2 mt-md-0" @click="goBack" variant="secondary">{{ $t('task-settings-delete-confirm-no') }}</b-btn>
    </b-container>
  </div>
</template>
