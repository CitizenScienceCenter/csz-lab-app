  ...
  <div>
    ...
    <b-collapse id="amazon-s3-collapse" v-model="isAmazonS3Visible" class="mb-3">
      ...
      <b-form ref="search-form" @submit.prevent="search">
        ...
        <b-button type="submit" ref="btn-search" class="mt-3" variant="secondary">{{ $t("taks-import-amazon-text1") }}</b-button>
        <b-button ref="btn-add-files" @click="addFiles" v-if="selectedFiles.length > 0" class="mt-3 float-right" variant="secondary">{{ $t("taks-import-amazon-text2") }}</b-button>
      </b-form>
